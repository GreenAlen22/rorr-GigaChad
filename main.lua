...
local initialize = function()
    ...
    local sprites = {
        ...
        decoy = Resources.sprite_load(NAMESPACE, "gigachad_decoy", path.combine(PATH, "gigachad", "decoy.png"), 1, 15, 10),
    }
    ...
    local sprSparksSpike = Resources.sprite_load(NAMESPACE, "gigachad_sparks", path.combine(PATH, "gigachad", "hitspike.png"), 4, 16, 17)
    local sprImpact = Resources.sprite_load(NAMESPACE, "gigachad_impact", path.combine(PATH, "gigachad", "impact.png"), 12, 16, 16, 0.7)
    ...
    gigachad:set_cape_offset(-3, -36, 2, -35) -- Set the Prophet cape offset for the player
        ...
        if (data.spikes == 3 and actor.image_index >= 6.0) or (data.spikes == 2 and actor.image_index >= 10.0) or (data.spikes == 1 and (actor.image_index >= 14.0 or actor.image_index >= 13.9)) then
            ...
            if actor:is_authority() then
                if not actor:skill_util_update_heaven_cracker(actor, damage) then
                    ...
                    for i=0, GM.get_buff_stack(actor, buff_shadow_clone) do
                        local attack = actor:fire_explosion(
                            actor.x + gm.cos(gm.degtorad(actor:skill_util_facing_direction())) * 105,
                            actor.y, 
                            150, 
                            120, 
                            damage, 
                        ...
                        local attack = actor:fire_explosion(
                            actor.x + gm.cos(gm.degtorad(actor:skill_util_facing_direction())) * -20,
                            actor.y, 
                            40, 
                            50, 
                            damage, 
    ...
    skill_moment:set_skill_properties(0, 4 * 60)
    ...
    state_moment:onStep(function(actor, data)
        ...
        if data.dodged == 0 and actor.image_index >= 6.0 then
            ...
            actor.pHspeed = GM.cos(GM.degtorad(actor:skill_util_facing_direction())) * actor.pHmax * 2.5
        end
        actor:skill_util_exit_state_on_anim_end()
    end)
            ...
            if not actor:skill_util_update_heaven_cracker(actor, damage) then 
                ...
                local function fire_special_attack(actor, buff_shadow_clone, damage)
                    ...
                    for i = 0, GM.get_buff_stack(actor, buff_shadow_clone) do
                        ...
                        -- Play a sound at the player's location
                        -- (sound_id, volume, pitch)
                        actor:sound_play(gm.constants.wClayShoot1, 0.2, 0.4 + math.random() * 0.2)
                    end
                end   
    ...
    --[[
        Subsection alt Primary Skill 
    ]]--
    -- Setup the alt Primary skill sprite, as well damage and cooldown properties
    skill_sparta:set_skill_icon(sprSkills, 5) 
    skill_sparta:set_skill_properties(5, 60)
    -- the onActivate callback is called when the player tries to use its alt primary skill
